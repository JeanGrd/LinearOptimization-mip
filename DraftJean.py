...
combinaisons = valid_combinations(lengths, 190, 255)
...
m += xsum(value * vars[key] for key, value in combinaison_contains(inconnus_combinaisons, 'A').items()) >= 200
m += xsum(value * vars[key] for key, value in combinaison_contains(inconnus_combinaisons, 'B').items()) >= 150
m += xsum(value * vars[key] for key, value in combinaison_contains(inconnus_combinaisons, 'C').items()) >= 215
m += xsum(value * vars[key] for key, value in combinaison_contains(inconnus_combinaisons, 'D').items()) >= 180
m += xsum(value * vars[key] for key, value in combinaison_contains(inconnus_combinaisons, 'E').items()) >= 150
m += xsum(value * vars[key] for key, value in combinaison_contains(inconnus_combinaisons, 'F').items()) >= 100

m += xsum(vars[indice] for indice, x in enumerate(inconnus_combinaisons)) <= 1000

m += xsum(vars) <= 100 + xsum(value * vars[key] for key, value in combinaison_contains(inconnus_combinaisons, 'A').items())
